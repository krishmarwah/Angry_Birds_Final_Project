...
public class NewPageScreen2 implements Screen {
    ...
    public NewPageScreen2(Game game) {
        this.game = game;
        shapeRenderer = new ShapeRenderer();
        font = new BitmapFont();  // Initialize BitmapFont
        newPageBackground = new Texture("new_image2.png");  // Load the background image
    ...
    public void render(float delta) {
        ...
        // Define central region dimensions
        float centerX = screenWidth / 2;
        float centerY = screenHeight / 2;
        float centerRadius = 50;  // Radius for central touch detection
        float leftOffset = 100;   // Distance left of center for left region

        // Check for click events
        if (Gdx.input.isButtonPressed(com.badlogic.gdx.Input.Buttons.LEFT)) {
            float touchX = Gdx.input.getX();
            float touchY = screenHeight - Gdx.input.getY();  // Invert Y-coordinate
            ...
            if (touchX >= playButtonX && touchX <= playButtonX + playButtonTexture.getWidth()
                ...
                game.setScreen(new GameScreen2(game));  // Switch to GameScreen when Play button is clicked
            }
            ...
            // Center area check for NewPageScreen1
            if (Math.sqrt(Math.pow(touchX - centerX, 2) + Math.pow(touchY - centerY, 2)) <= centerRadius) {
                game.setScreen(new NewPageScreen1(game));  // Switch to NewPageScreen1 when touched at the center
            }

            // Slightly left of center area check for NewPageScreen
            if (touchX >= centerX - leftOffset && touchX < centerX
                && Math.abs(touchY - centerY) <= centerRadius) {
                game.setScreen(new NewPageScreen(game));  // Switch to NewPageScreen when touched left of center
            }
        }
    }
