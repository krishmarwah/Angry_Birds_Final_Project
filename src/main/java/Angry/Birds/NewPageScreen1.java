...
public class NewPageScreen1 implements Screen {
    ...
    public NewPageScreen1(Game game) {
        this.game = game;
        shapeRenderer = new ShapeRenderer();
        font = new BitmapFont();  // Initialize BitmapFont
        newPageBackground = new Texture("new_image1.png");  // Load the background image
    ...
    public void render(float delta) {
        ...
        if (Gdx.input.isButtonPressed(com.badlogic.gdx.Input.Buttons.LEFT)) {
            ...
            String centerText = "Touch Here to Return";
            float centerX = Gdx.graphics.getWidth() / 2-80;
            float centerY = Gdx.graphics.getHeight() / 2-20;
            float centerRadius = 80;// Define a radius around the center to detect touch


            if (Math.sqrt(Math.pow(touchX - centerX, 2) + Math.pow(touchY - centerY, 2)) <= centerRadius) {
                game.setScreen(new NewPageScreen(game));  // Switch back to NewPageScreen when touched near the center
            }
        }
    }
